	...
	return (
		<div {...props} class="w-full flex flex-col lg:flex-row items-start mb-10" >
			<div class="w-full max-w-[16rem] flex lg:flex-col justify-start lg:justify-end lg:pr-10 items-center lg:items-end">
				<span class="text-[#0009] text-base lg:text-[1.1rem] mb-1">
					{props.task.date}
				</span>
			...
			<div class="w-full flex flex-col justify-start items-start">
				<span class="font-bold text-lg lg:text-[1.2rem] border-b-1 border-[#594f4f] border-dotted">
					{props.task.title}
				</span>
				<div class="w-full lg:hidden flex justify-start items-center gap-2">
					{props.task.links.map(({ href, name }) => <a href={href} class="text-blue-900 font-medium" key={href} >{name}</a>)}
				</div>
				<span class="font-medium text-base lg:text-[1rem] mt-1 mb-2 text-[#030303]">
